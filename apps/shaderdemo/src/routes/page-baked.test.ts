...
import defaultSettings from '@mia-cx/atmosphere/default-settings.json';
import { BAKED_RENDER_OPTIONS, loadBakedRenderOptions } from '@mia-cx/atmosphere/baked-settings';
import { composeAdjustmentLut } from '@mia-cx/atmosphere/adjustments';
import { leadingAdjustmentRegion, rendererStagePlan } from '@mia-cx/atmosphere/pipeline';
...
describe('baked shader-only page', () => {
    ...
    it('uses canonical JSON defaults directly and generated immutable products', async () => {
        ...
        const bytes = readFileSync(
            new URL('../../../../packages/atmosphere/src/baked-adjustment-lut.bin', import.meta.url),
        );
    ...
    it('bypasses immutable per-frame JSON keys in both backends without removing GPU adaptation evidence', () => {
        const webgpu = readFileSync(
            new URL('../../../../packages/atmosphere/src/renderer.ts', import.meta.url),
            'utf8',
        );
        const webgl = readFileSync(
            new URL('../../../../packages/atmosphere/src/webgl2-renderer.ts', import.meta.url),
            'utf8',
        );
